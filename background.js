  ...
  if (msg.type === "REFRESH_CATALOG") {
    loadCatalog()
      .then(async (result) => {
        ...
        if (!result.ok && result.cloudflare) await showSiteTab();
        sendResponse(result);
      })
      .catch((error) => {
        sendResponse({ ok: false, message: error?.message || "Не удалось обновить списки" });
      });
    return true;
  }
...
async function loadCatalog() {
  ...
  try {
    ...
    return {
      ...
      message: timedOut
        ? "Кабинет не ответил за 20 секунд. Проверьте вход на l2reborn.org и повторите."
        : (error.message || "Нет связи с l2reborn.org")
    };
  }
  if (!payload?.success) {
  ...
  const message = summarizeCatalog(catalog);
  await chrome.storage.local.set({ catalog, catalogAt: Date.now() });
  await setStatus({ state: accounts.length || characters.length ? "ok" : "waiting", message });
  return { ok: true, catalog, message };
}

function summarizeCatalog(catalog) {
  const accounts = Object.values(catalog.byServer || {}).flat();
  const characters = accounts.reduce((count, account) => count + (account.characters?.length || 0), 0);
  if (!accounts.length) return "Кабинет ответил, но аккаунтов и персонажей нет.";
  return "Обновлено: аккаунтов " + accounts.length + ", персонажей " + characters + ".";
}
...
function readJson(result) {
  ...
  try {
    ...
    throw new Error("Сайт вернул страницу вместо JSON (HTTP " + result.status + "). Обновите вкладку l2reborn.org и повторите.");
  }
}
...
function isChallenge(result) {
  ...
  return result.status === 403 || text.startsWith("<") || text.startsWith("<!");
}
